...
\newpage
Wie in der Abbildung (Abb. \ref{fig:architektur}) erkenntlich besteht diese Diplomarbeit aus Komponenten, welche sich in drei primäre Kategorien einteilen lassen: Backend- (Cyan), Frontend- (Violett) und Infrastruktur-Services (Rot). \\

Darüber hinaus bestehen Anbindungen an externe Services (Grau), wie etwa die Plate Recognizer SDK.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{web_interactions.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{grafana_to_db.png}
...
Sie speichert pro Benutzer den Namen und die E-Mail-Adresse sowie zwei Flags, welche festlegen, ob der Benutzer Echtzeit-Alerts oder periodische Zusammenfassungen per E-Mail erhalten möchte.

Die folgende Darstellung (Abb. \ref{fig:db_tables}) zeigt die Attribute der beiden Entitäten als ER-Diagramm:
...
HTTP Basic Authentication ist dabei die einzige von der Surveillance Station unterstützte Authentifizierungsmethode für Webhook-Aufrufe.
...
Wie die Darstellung (Abb. \ref{fig:db_user}) zeigt, hat der Data-Collection-User Vollzugriff auf das Ingestion-Schema, der Analytics-User nur Lesezugriff darauf (aber Vollzugriff auf das eigene Analytics-Schema), und der Notification-User ist vollständig auf das Notification-Schema beschränkt.
