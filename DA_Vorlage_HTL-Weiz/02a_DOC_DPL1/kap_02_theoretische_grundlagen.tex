...
Die Containerisierung ist eine Methode, bei welcher Anwendungen und deren Abhängigkeiten in einer leichtgewichtigen, isolierten Einheit, einem sogenannten Container, verpackt werden. \cite{aws_containerization}

\subsubsection{Docker}
Docker ist eine offene Plattform, welche 2013 veröffentlicht wurde und die Containerisierung von Anwendungen standardisiert.
...
Die Docker-Plattform besteht aus der Docker Engine (Daemon, REST-API und CLI), Docker Images und Docker Containern. \cite{docker_overview}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=8cm]{docker-logo-blue.png}
    \caption{Docker Logo}
...
Der Docker-Daemon verwaltet Container, Images, Netzwerke und Volumes (persistenter Speicher), während der Docker-Client (CLI) die Schnittstelle für Benutzerinteraktionen bereitstellt. \cite{docker_overview} \\
Images, also Abbilder der Anwendung und ihrer Umgebung, werden als unveränderliche Vorlagen aus sogenannten Dockerfiles erstellt und in Layern organisiert, wobei bestimmte Instruktionen wie \texttt{RUN} oder \texttt{COPY} im Dockerfile jeweils einen neuen Layer erzeugen. \cite{docker_dockerfile}
Dieses Layer-System ermöglicht effizientes Caching: Wenn ein Image neu gebaut werden muss, müssen nur die Layer neu erstellt werden, an denen tatsächlich Änderungen vorgenommen wurden. \cite{docker_cache}

Container werden aus diesen Images instanziiert und stellen isolierte Prozesse und deren Umgebung dar.
Im Gegensatz zu virtuellen Maschinen, die jeweils ein vollständiges Gastbetriebssystem benötigen, teilen sich Container den Host-Kernel, was sie deutlich ressourcenschonender macht. \cite{docker_overview}

\subsubsection{Vorteile von Containerisierung und Docker}
Ein Punkt, welcher für Containerisierung spricht, ist die Eliminierung des klassischen "`Works on my machine"'-Problems.
...
Ohne Nutzung von Containerisierung müssten die einzelnen Services, inklusive ihrer Abhängigkeiten und Konfigurationen, entweder als Programme verpackt werden (zum Beispiel als Windows .exe-File)
...
Abhängigkeiten oder Python-Pakete (etwa definiert in einem \texttt{uv.lock}-File) eines Services können nicht mit denen eines anderen kollidieren.
...
Statt mehrere Services manuell zu starten, orchestriert eine einzige Datei das komplette System, was auch unterschiedliche Umgebungen wie etwa Entwicklung und Live (Produktion) durch separate Compose-Dateien ermöglicht. \cite{docker_compose}
...
Es enthält eine Vielzahl an vorgefertigten Images, darunter offizielle Images für gängige Software und Technologien wie PostgreSQL, Redis oder Python, welche von den jeweiligen Maintainern verifiziert werden. \cite{docker_hub}
...
\subsubsection{Grundkonzept und Definition}
...
Dieses Prinzip profitiert stark von der Containerisierung, welche im letzten Kapitel erklärt wurde. \cite{aws_microservices}
...
\subsubsection{Wartbarkeit und Verständlichkeit}
...
und die gesamte Infrastruktur kann als Code versioniert und reproduzierbar deployed werden (siehe Kapitel~\ref{sec:docker_compose}).
...
Zudem führt die notwendige Inter-Service-Kommunikation über das Netzwerk im Vergleich zu monolithischen Architekturen zu zusätzlichen Latenzen.

Im Fazit ist die Microservice-Architektur ein mächtiger Ansatz für komplexe Systeme, der Flexibilität, Fehlertoleranz und Skalierbarkeit bietet und in dieser Diplomarbeit vor allem in Bezug auf Fehlertoleranz, wie es oben beschrieben wurde, einen großen Vorteil mit sich bringt. \cite{ibm_microservices}

\newpage
\subsection{ALPR (Automatic License Plate Recognition)}
\label{sec:alpr}

ALPR (Automatic License Plate Recognition) bezeichnet die automatisierte Erfassung und Auswertung von Fahrzeugkennzeichen aus Bilddaten mittels Computer-Vision und Deep Learning. 
ALPR-Systeme arbeiten kontinuierlich und transformieren visuelle Informationen wie Kamera-Snapshots in strukturierte, maschinell verarbeitbare Daten und ermöglichen damit die automatische Identifikation von Fahrzeugen. \cite{platerecognizer_alpr}

\subsubsection{Grundfunktionsweise}
Der Erkennungsprozess und die Verarbeitung der Daten erfolgt typischerweise in einem mehrstufigen Prozess \cite{senstar_alpr}:

\begin{enumerate}
    \item \textbf{Detection (Lokalisierung):}
    ...
    \item \textbf{Transformation (Normalisierung):}
    ...
    \item \textbf{Post-Processing (Validierung):}
...
\newpage
\subsubsection{Technologie und Anbieter}
...
\subsubsection{MMC-Erkennung (Make, Model, Color)}
\label{sec:mmc}
Neben der im Vergleich sehr zuverlässigen Kennzeichenerkennung identifiziert Plate Recognizer zusätzliche Fahrzeugattribute, ein Ansatz, der MMC-Erkennung genannt wird \cite{platerecognizer_mmc}:
\begin{itemize}
    \item \textbf{Make (Marke):} Identifikation des Fahrzeugherstellers basierend auf charakteristischen Designmerkmalen.
    \item \textbf{Model (Modell):} Feinere Unterscheidung innerhalb einer Marke, etwa zwischen verschiedenen Baureihen oder Generationen.
    \item \textbf{Color (Farbe):} Bestimmung der Primärfarbe unter Kompensation von Lichtverhältnissen und Unterscheidung zwischen verschiedenen Lackierungstypen.
\end{itemize}

\subsubsection{On-Premise SDK}
\label{sec:on_premise}
Plate Recognizer bietet sowohl Cloud-basierte APIs als auch ein On-Premise SDK an.
Die On-Premise-Variante ermöglicht vollständige lokale Ausführung ohne externe Netzwerkabhängigkeit, eine Anforderung, welche Zotter Schokolade sehr wichtig war. \cite{platerecognizer_snapshot}

\paragraph{Vorteile der lokalen Ausführung:}
